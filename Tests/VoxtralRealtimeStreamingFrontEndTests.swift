...
struct VoxtralRealtimeStreamingFrontEndTests {
    ...
    /// A stream that ended on `maxTokens` reads no more audio. Before, every later
    /// `step` still appended its samples to a buffer nothing consumed, so a host that
    /// kept streaming grew memory without bound.
...
/// The in-place cache must give attention exactly what the concatenate-then-trim cache
/// it replaced gave: the same rows, in the same order, at the same position offset.
struct VoxtralRealtimeDecoderKVCacheTests {
    @Test func windowsMatchConcatenateAndTrimAcrossGrowthAndCompaction() {
        ...
        let cache = VoxtralRealtimeDecoderKVCache(
            keys: prefill, values: prefill + 0.5, slidingWindow: slidingWindow
        )
        ...
        // 1 505 rows went in. The storage grew once, from one block to two, and
        // compaction kept it there.
