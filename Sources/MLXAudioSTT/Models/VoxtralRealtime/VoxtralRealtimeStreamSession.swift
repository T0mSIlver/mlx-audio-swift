...
public final class VoxtralRealtimeStreamSession {
    ...
    /// out when the parent is more than 16 KB larger than them, which frees the dropped
    /// prefix. Below that MLX shares the parent, and the next step's concatenation
    /// copies the kept rows into a new buffer anyway.
    ///
