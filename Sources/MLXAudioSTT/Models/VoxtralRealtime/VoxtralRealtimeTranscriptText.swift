...
/// decoded.
///
/// The session used to turn its whole token list back into text after every step, so a
/// step's cost grew with the transcript. This type keeps the finished text plus the
/// first bytes of a character whose remaining bytes are still to come, at most three,
/// and an append costs only the new bytes.
///
...
struct VoxtralRealtimeTranscriptText {
    ...
    ///
    /// Reading it returns `settled` without copying, except while a character is
    /// incomplete: then it builds a new string as long as the transcript. Hosts that
    /// only need what each step added should use `Delta.text`.
    var text: String {
    ...
    /// The text added since `mark`, reported the way the session always reported it by
    /// comparing whole transcripts: the new suffix when the old transcript is a prefix
    /// of the new one, and the whole new transcript when it is not. That happens when a
    /// trailing U+FFFD became a real character, or when new text combined with the last
    /// character into one.
    ///
    /// Only the last settled character at `mark` and what follows it can compare
    /// differently, so only that tail is compared, and the cost does not grow with the
    /// transcript. The whole-transcript result is returned only in the rare mismatch.
